...
$(O)/boot.fat: $(O)/kernel.elf grub/grub.cfg grub/grub.efi $(O)/writeok
	@echo "  GEN    $@"
	$(Q)dd if=/dev/zero of=$@ bs=4096 count=1536000 2> /dev/null
	...
	$(Q)mcopy -s -i $@ /home/jonathan/tmp/qemu ::qemu
	$(Q)mcopy -s -i $@ $(O)/fs/* ::
$(O)/boot.img: $(O)/boot.fat $(O)/fs.part grub/boot.img grub/core.img
	...
	$(Q)PARTED_GPT_APPLE=0 parted -s --align minimal $@ mklabel gpt \
		...
		mkpart primary 33MiB 6033MiB set 1 legacy_boot on set 1 esp on
